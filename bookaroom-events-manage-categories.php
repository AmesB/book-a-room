...
{
	public static function showFormCategories() {		
		...
		switch( $externals['action'] ):
			case 'addCheck':
				...
				self::showForm( NULL, "You have successfully added the category {$externals['newName']}" );
				break;
			...
			case 'deactivateFinal':
				if( ( $error = self::checkHash( $externals['hash'], $externals['time'], $externals['groupID'], 'active' ) ) == TRUE ):
					...
					self::showForm( $externals, 'You have successfully deactivated a category.' );
				endif;
				break;
	...
	protected static function showActivateChangeCheck( $Name, $groupID, $type, $errorMSG = NULL )
	{
		switch( $type ):
			case 'deactivate':
				$filename = BOOKAROOM_PATH . 'templates/events/categories_form_deactivate.html';	
				break;
			case 'reactivate':
				$filename = BOOKAROOM_PATH . 'templates/events/categories_form_reactivate.html';	
				break;
			default:
		...
		$contents = str_replace( '#groupName#', $Name, $contents );
		$contents = str_replace( '#groupID#', $groupID, $contents );
		
		# make hash
		$time = time();
		
		$hash = md5( $groupID . $time . $Name );
		...
		if( ( $isDupe = array_search( strtolower( $newName ), $nameList['all'] ) ) == TRUE ):
			...
			if( $groupID == $isDupe ):
				...
				if( true == $nameList['status'][$isDupe] ):
					return "The name you've entered is in use in an <em><strong>active</strong></em> category already.";
				else:
					return "The name you've entered is in use in an <em><strong>inactive</strong></em> category already. You can reactivate it in the bottom menu.";
				endif;
	...
	protected static function checkID ( $groupID, $action )
	{
		...
		switch( $action ):
			...
			case 'deactivate':
				if( !array_key_exists( $groupID, $nameList['active'] ) ):
					return 'This category is already inactive. You cannot deactivate it.';
				endif;
				break;

			case 'reactivate':
				if( array_key_exists( $groupID, $nameList['active'] ) ):
					return 'This category is already active. You cannot reactivate it.';
				endif;
	...
	{
		$filename = BOOKAROOM_PATH . 'templates/events/categories_form_edit.html';	
		$handle = fopen( $filename, "r" );
		$contents = fread( $handle, filesize( $filename ) );
		fclose( $handle );
	...
	protected static function moveGroup( $moveType, $moveID )
	{
		...
		if( $nameList['status'][$moveID] == FALSE ):
			return 'The category you\'re trying to move is inactive and can\'t be reordered.';
		endif;
		...
		if( in_array( $moveType, array( 'moveUp', 'moveTop' ) ) ): 		
			if( ( $curPos = array_search( $moveID, $curList ) ) == 0 ):
				return 'The category you\'re trying to move is the first in the list and can go no higher.';
			endif;
		endif;
		...
		if( in_array( $moveType, array( 'moveDown', 'moveBottom' ) ) ): 
			if( ( $curPos = array_search( $moveID, $curList ) ) == ( count( $curList ) - 1 ) ):
				return 'The category you\'re trying to move is the last in the list and can go no lower.';	
			endif;
		endif;
	...
	protected static function checkNewCategory( $newCategoryName, $curID = NULL )
	{
		$errors = array();
		$errorMSG = FALSE;
		...
		if( empty( $newCategoryName ) ):
			$errors[] = "You haven't entered a new category. Please try again.";
		else:
			...
			if( $dupeID = array_search( strtolower( trim( $newCategoryName ) ), $nameList['all'] ) ):
				if( true == $nameList['status'][$dupeID] ):
					$errors[] = "The name you've entered is in use by a <em><strong>active</strong></em> category already.";
				else:
					$errors[] = "The name you've entered is in use by a <em><strong>inactive</strong></em> category already. You can reactivate it in the bottom menu.";
				endif;
			# remember to filter against $curID if not empty so you		
			endif;
	...
	{
		$filename = BOOKAROOM_PATH . 'templates/events/categories_form.html';	
		$handle = fopen( $filename, "r" );
		$contents = fread( $handle, filesize( $filename ) );
		fclose( $handle );
		...
		if( count( $nameList['active'] ) == false ):
			...
			foreach( $keyList as $key => $val ):
				...
				$temp = str_replace( '#groupName#', $nameList['active'][$val]['categories_desc'], $temp );
				$temp = str_replace( '#groupID#', $nameList['active'][$val]['categories_id'], $temp );
				$final[] = $temp;
			endforeach;
		...
		if( count( $nameList['inactive'] ) == false ):
			...
			foreach( $keyList as $val ):
				$temp = $inactive_line;
				$temp = str_replace( '#groupName#', $nameList['inactive'][$val]['categories_desc'], $temp );
				$temp = str_replace( '#groupID#', $nameList['inactive'][$val]['categories_id'], $temp );
				$final[] = $temp;
			endforeach;
	...
	protected static function makeLinks( $groupID, $curPos, $topPos )
	{
		...
		switch( $curPos):
			case 0:
				...
				$link[] = 'Top';
				$link[] = 'Move Up';
				$link[] = "<a href=\"?page=bookaroom_event_settings_categories&action=moveDown&groupID={$groupID}\">Move Down</a>";
				$link[] = "<a href=\"?page=bookaroom_event_settings_categories&action=moveBottom&groupID={$groupID}\">Bottom</a>";
				break;
				
			case ( $curPos == ( $topPos - 1 ) ):
				# bottom line
				$link[] = "<a href=\"?page=bookaroom_event_settings_categories&action=moveTop&groupID={$groupID}\">Top</a>";
				$link[] = "<a href=\"?page=bookaroom_event_settings_categories&action=moveUp&groupID={$groupID}\">Move Up</a>";
				$link[] = 'Move Down';
				$link[] = 'Bottom';
				break;
